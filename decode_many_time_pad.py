cts  = ["315c4eeaa8b5f8aaf9174145bf43e1784b8fa00dc71d885a804e5ee9fa40b16349c146fb778cdf2d3aff021dfff5b403b510d0d0455468aeb98622b137dae857553ccd8883a7bc37520e06e515d22c954eba50", 
...
for i in range(len(cts)):
    cts_2.append([cts[i][j:j+2] for j in range(0, len(cts[i]), 2)])

count = [([0] * len(cts_2[0])) for _ in range(10)]

for i in range(10):
    for j in range(i+1, 10):
        for k in range(len(cts_2[0])):
            number = (int(cts_2[i][k], 16) ^ int(cts_2[j][k], 16)) ^ 32
            # BIG case
...
for i in range(0, len(target), 2):
    target_2.append(target[i:i+2])
ans = []

# create which_c
which_c = [0] * len(target_2)
for i in range(len(target_2)):
    best_c = 0
    for j in range(10):
        if count[j][i] > count[best_c][i]:
            best_c = j
    which_c[i] = best_c
...
for i in range(len(target_2)):
    ...
    if c == -1:
        ...
        decode = (int(cts_2[c][i], 16) ^ int(target_2[i], 16)) ^ 32
    ans.append(decode)
print(ans)
